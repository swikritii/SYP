...
export default function CourtDetails() {
  ...
  const prevImage = () => {
    if (court) setCurrentImageIndex((prev) => (prev - 1 + court.images.length) % court.images.length);
  };

  const handleBooking = async (bookingData) => {
    try {
        // Parse time to a 24-hour HH:MM:00 format
        const timeParts = bookingData.time.match(/(\d+):(\d+) (AM|PM)/);
        ...
        const startTime = `${hours.toString().padStart(2, '0')}:${mins}:00`;
        const endHours = hours + bookingData.duration;
        const endTime = `${(endHours % 24).toString().padStart(2, '0')}:${mins}:00`; // modulo 24 just in case
        ...
        if (res.is_free_reward) {
            alert("Congratulations! This is your 10th booking and it is FREE!");
        }

        navigate('/booking-confirmation', { state: { booking: res } });
    } catch (err) {
        alert(err.message || 'Failed to book court');
    }
  };
  ...
  if (loading) return <div className="text-center py-20">Loading court details...</div>;
  if (!court) return <div className="text-center py-20 text-red-500">Court not found</div>;
